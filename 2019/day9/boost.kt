...
fun Splitcodes(opcodes: String): List<Long> {
    // println("you gave ops of $opcodes");
    var ops: List<Long> = opcodes.split(",").map{ it.trim().toLong() };
    // println("This is ${ops.size} instructions");
    return ops;
}


class Boost constructor (sourceCodes: List<Long>, _inputs: MutableList<Long>, _outputs: MutableList<Long>) {
...
fun main(args: Array<String>) {
    ...
    inputs.add(2);
    var boost = Boost(sourceCodes, inputs, outputs);
    do {
        boost.ops();
    } while (!boost.isExited());
    println(boost.outputs);
    // println(boost.ops);
}
